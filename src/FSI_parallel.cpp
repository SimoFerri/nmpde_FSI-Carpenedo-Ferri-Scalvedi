...
namespace Step46
{
  ...
  public:
    ...
    void vmult(TrilinosWrappers::MPI::BlockVector &dst, const TrilinosWrappers::MPI::BlockVector &src) const
    {
      // SOLVE FLUID VELOCITY
      {
        ...
        TrilinosWrappers::SolverGMRES solver_gmres_vel(solver_control_vel);
        dst.block(0) = 0;
      ...
      tmp_p.sadd(-1.0, 1.0, src.block(1)); // tmp_p = - B * dst.block(0) + src.block(1)
      {
        ...
        TrilinosWrappers::SolverCG solver_cg_pres(solver_control_pres);
        dst.block(1) = 0;
      ...
      tmp_d.sadd(-1.0, 1.0, src.block(2)); // tmp_d = - Cu * dst.block(0) - Cp * dst.block(1) + src.block(2)
      {
        ...
        TrilinosWrappers::SolverGMRES solver_gmres_disp(solver_control_disp);
        dst.block(2) = 0;
  ...
  void FluidStructureProblem<dim>::make_grid()
  {
    ...
    triangulation.set_mesh_smoothing(Triangulation<dim>::maximum_smoothing);
  }
  ...
  void FluidStructureProblem<dim>::setup_dofs() // EDITED
  {
    ...
    system_rhs.reinit(block_owned_dofs, MPI_COMM_WORLD); // 3 INSTEAD OF 2
  }
  ...
  float FluidStructureProblem<dim>::refine_mesh(float tol)
  {
    ...
    float stokes_norm = stokes_estimated_error_per_cell.norm_sqr();
    float global_stokes_norm = std::sqrt(Utilities::MPI::sum(stokes_norm, MPI_COMM_WORLD));

    stokes_estimated_error_per_cell *=
      4.0f / global_stokes_norm;

    float elasticity_norm = elasticity_estimated_error_per_cell.norm_sqr();
    float global_elasticity_norm = std::sqrt(Utilities::MPI::sum(elasticity_norm, MPI_COMM_WORLD));

    elasticity_estimated_error_per_cell *=
      1.0f / global_elasticity_norm;
