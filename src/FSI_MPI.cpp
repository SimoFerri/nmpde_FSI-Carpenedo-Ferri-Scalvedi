...
namespace Step46
{
  using namespace dealii;
                    ...
                    const std::vector<std::vector<bool>> &displacement_constant_modes = {})
    {
        ...
        // Initialize tmp vectors using the matrix's own partitioning
 ...
 
    {
        ...
        for (const auto &cell : dof_handler.active_cell_iterators()) {
            ...
            for (const auto f : cell->face_indices()) {
                ...
                for (unsigned int sub = 0; sub < cell->face(f)->n_children(); ++sub) {
                    ...
                    constraints.add_entries_local_to_global(local_dofs, neighbor_dofs, dsp);
                    }
                }
                } else if (cell_is_in_fluid_domain(cell->neighbor(f))) {
                neighbor_dofs.resize(cell->neighbor(f)->get_fe().n_dofs_per_cell());
        ...
        // Pressure mass matrix sparsity (pressure-pressure only)
        ...
        if (cell_is_in_fluid_domain(cell)) {
            ...
            for (unsigned int i = 0; i < pressure_local_indices.size(); ++i)
                ...
                pressure_submatrix(i, j) =
                    local_pressure_mass(pressure_local_indices[i], pressure_local_indices[j]);
            constraints.distribute_local_to_global(pressure_submatrix,
                                                    pressure_global_indices, mass_matrix);
        }
  ...
  void FluidStructureProblem<dim>::solve()
  {
    ...
    pcout << "   Converged in " << solver_control.last_step() << " iterations." << std::endl;
  }
    ...
    const unsigned int refinement_cycle) const
  {
    ...
    data_out.write_vtu_with_pvtu_record("./", "solution-fsi", refinement_cycle, MPI_COMM_WORLD);
  }
  ...
  float FluidStructureProblem<dim>::refine_mesh(float tol)
  {
    ...
    // Estimate error for displacement
    const FEValuesExtractors::Vector displacements(dim + 1);
            KellyErrorEstimator<dim>::estimate(
            dof_handler,
            face_q_collection,
    ...
    // Scale stokes error globally across all MPI processes
    float local_sq = stokes_estimated_error_per_cell.norm_sqr();
    float global_sq = 0.0f;
    // Each process only holds local error entries, so we must reduce
    // the norm across all processes before scaling
    MPI_Allreduce(&local_sq, &global_sq, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    stokes_estimated_error_per_cell *= 4.0f / std::sqrt(global_sq);

    // Scale elasticity error globally
    local_sq = elasticity_estimated_error_per_cell.norm_sqr();
    global_sq = 0.0f;
    MPI_Allreduce(&local_sq, &global_sq, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    elasticity_estimated_error_per_cell *= 1.0f / std::sqrt(global_sq);

    // Combine the two error estimates
    ...
    // Zero out error at fluid-solid interface cells to avoid artificially
    // large indicators on both sides of the interface
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
        if (!cell->is_locally_owned()) continue;
        ...
        if (cell_is_in_solid_domain(cell))
        {
            if ((cell->at_boundary(f) == false) &&
                        ...
                        ((cell->neighbor(f)->level() == cell->level()) &&
                        (cell->neighbor(f)->has_children() == true) &&
                        (cell_is_in_fluid_domain(cell->neighbor_child_on_subface(f, 0)))) ||
        ...
        else
        {
            if ((cell->at_boundary(f) == false) &&
                        ...
                        ((cell->neighbor(f)->level() == cell->level()) &&
                        (cell->neighbor(f)->has_children() == true) &&
                        (cell_is_in_solid_domain(cell->neighbor_child_on_subface(f, 0)))) ||
                        (cell->neighbor_is_coarser(f) &&
                        cell_is_in_solid_domain(cell->neighbor(f)))))
            estimated_error_per_cell(cell->active_cell_index()) = 0;
        }
    ...
    // Compute the global error norm across all MPI processes: we cannot use 
    // estimated_error_per_cell.l2_norm() directly because each process only holds its 
    // local portion of the vector, so the result would differ per process and 
    // the tolerance check would be inconsistent
    float local_norm_sq = estimated_error_per_cell.norm_sqr();
    float global_norm_sq = 0.0f;
    MPI_Allreduce(&local_norm_sq, &global_norm_sq, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
  ...
  void FluidStructureProblem<dim>::run(const unsigned int max_cycles, const float tol)
  {
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle < max_cycles;
         ++refinement_cycle)
      {
        ...
        // Attempt point_value on whichever process owns the cell
        ...
        // Reduce: sum across all processes so rank 0 gets the correct value
        // Only the owning process has nonzero entries, all others have zero
        ...
        for (unsigned int i = 0; i < fe_collection.n_components(); ++i)
        {
            ...
            MPI_Allreduce(&local_entry, &global_entry, 1,
                        MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            global_value[i] = global_entry;
        }

        // Now rank 0 has the correct value and pcout will print it
        ...
        for (unsigned int i = dim + 1; i < fe_collection.n_components(); ++i)
            pcout << global_value[i] << (i < fe_collection.n_components() - 1 ? ", " : "");
        pcout << "]" << std::endl;
